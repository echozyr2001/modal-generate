...
class MusicGenServer:
    """Unified music generation server"""
    ...
    def generate_music(self, request: MusicGenerationRequest) -> MusicGenerationResponseBase64:
        """Generate music and return as base64"""
    ...
    @modal.fastapi_endpoint(method="POST")
    def generate_music_to_storage(self, request: MusicGenerationRequest) -> MusicGenerationResponseFile:
        """Generate music and save to storage"""
        operation_id = self.generate_operation_id()
        start_time = time.time()
        
        logger.info(f"[{operation_id}] Music generation to storage: {request.prompt[:100]}...")
        
        # Start monitoring
        self.start_operation(operation_id, "music_generation_storage")
        ...
        try:
            ...
            logger.info(f"[{operation_id}] Music saved to storage: {file_key}")
            
            return MusicGenerationResponseFile(
                file_path=file_key,
                generation_time=time.time() - start_time,
            ...
            self.cleanup_operation(operation_id)
            logger.error(f"[{operation_id}] Music generation to storage failed: {e}")
            raise HTTPException(status_code=500, detail="Music generation failed")
    
    @modal.fastapi_endpoint(method="POST")
    def generate_demo_music(self) -> MusicGenerationResponseBase64:
        """Generate demo music for testing"""
        operation_id = self.generate_operation_id()
        start_time = time.time()
...
def test_music_service():
    """Test the unified music service"""
    import requests
    import time
    
    server = MusicGenServer()
    
    print("=== Testing Unified Music Service ===")
